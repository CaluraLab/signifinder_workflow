...
# Prepare Data
In this workflow, we present all the analyses, along with the code, to investigate with `signifinder` the single-cell RNA sequencing samples of the glioblastoma dataset of Darmanis et al. (GEO ID: GSE84465, PMID 29091775). We download the dataset using the `TMExplorer` R package (version 1.6.2). The `SingleCellExperiment` object provides the raw count expression values.
...
Plot the t-SNE colored by cell type and cell location.
...
Plot t-SNE colored by scores of Hypoxia_Buffa, CellCycle_Davoli, ECM_Chakravarthy_up and ECM_Chakravarthy_down and the corresponding boxplots.
...
Plot t-SNE colored by scores of Hypoxia_Buffa, CellCycle_Davoli and VEGF_Hu for the neoplastic cells and the corresponding boxplots.
...
Plot t-SNE colored by scores of IPS_Charoentong_CP, IPS_Charoentong_EC, IPS_Charoentong_MHC, IPS_Charoentong_SC, ImmunoScore_Roh and Chemokines_Messina for the immune cells and the corresponding boxplots.
...
Generate correlation matrices between signature scores of VEGF_Hu, Hypoxia_Buffa and CellCycle_Davoli computed on the neoplastic cells and between signature scores of Chemokines_Messina, ImmunoScore_Roh and IPS_Charoentong computed on the immune cells.
...
Generate the scatterplot of CellCycle_Davoli scores and CIN_Carter_70 scores computed on the neoplastic cells.
