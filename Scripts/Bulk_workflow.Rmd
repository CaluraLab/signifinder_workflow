...
# Compute Signatures
We look for signatures useful for OVC investigation. Hence, those signatures specifically developed in OVC or signatures that are available for all cancer types (pan-cancer). Thus, we compute all of them.
...
Generate the heatmap to visualize scores for the signatures inside group 1, 2, 3, and 4.
...
Generate the ridge plots to visualize scores for the signatures inside group 1, 2, 3, and 4.
...
Plot survival curves for EMT_Miow_Mesenchymal signature.
...
Generate a scatterplot of scores and a barplot of their density distribution for ASC_Smith signature.

```{r}
oneSignPlot(data = OVse, whichSign = "ASC_Smith", statistics = "quantiles")
```

Generate a boxplot of the ASC_Smith signature scores divided by OV subtype.
