...
# Prepare Data
We downloaded the spatial transcriptomic dataset “Human Breast Cancer: Ductal Carcinoma In Situ, Invasive Carcinoma (FFPE)”, included in the 10x Genomics Visium Spatial Gene Expression data, from the 10x website (https://cf.10xgenomics.com/samples/spatial-exp/1.3.0/Visium_FFPE_Human_Breast_Cancer/Visium_FFPE_Human_Breast_Cancer_web_summary.html).
Annotation of the histology image was performed by an expert anatomopathologist using the image file (https://cf.10xgenomics.com/samples/spatial-exp/1.3.0/Visium_FFPE_Human_Breast_Cancer/Visium_FFPE_Human_Breast_Cancer_image.tif). Histological annotations of spots are then manually included in the data using 10x Genomics Loupe Browser.
Then, we imported the spot gene expressions (https://cf.10xgenomics.com/samples/spatial-exp/1.3.0/Visium_FFPE_Human_Breast_Cancer/Visium_FFPE_Human_Breast_Cancer_filtered_feature_bc_matrix.h5) and their histological annotations in the form of a `SpatialExperiment` using the `read10xVisium` function from the `SpatialExperiment` package (version 1.8.0). The object is available as BRCAspe.RData in the signifinder_workflow/Data directory.

As a first step of this workflow, we load the BRCAspe.RData `SpatialExperiment` object. Then, we normalize the raw counts using the `logNormCounts` function from the `scater` package (version 1.24.0) as described here below.
...
library(ggspavis)
plotCoords(
    spe = BRCAspe, annotate = "anno_spots", point_size = 1.6,
...
lapply(
    c("Hypoxia_Buffa", "EMT_Cheng", "EMT_Cheng_tumor", "CellCycle_Lundberg",
      ...
      "Chemokines_Messina"), function(x){
          plotCoords(spe = BRCAspe, annotate = x, point_size = 1) +
...
lapply(
    ...
    function(x){
        list(plotCoords(BRCAspe, annotate = paste0(x, "_lymphocytes"), point_size = 1) +
        ...
        plotCoords(BRCAspe, annotate = paste0(x, "_caf"), point_size = 1) +
            scale_colour_gradientn(
                colours = myPalette(100), na.value = "grey90",
                limits = c(min(colData(BRCAspe)[,x]),
                           max(colData(BRCAspe)[,x]))) +
        ...
        plotCoords(BRCAspe, annotate = paste0(x, "_tumor"), point_size = 1) +
            scale_colour_gradientn(
                colours = myPalette(100), na.value = "grey90",
                limits = c(min(colData(BRCAspe)[,x]),
                           max(colData(BRCAspe)[,x]))) +
